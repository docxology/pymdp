...
**32 validated examples** across 12 categories, all executed via `run_all.py` through `pkg.support.mirror_dispatch`. Every example automatically produces:

- `{stem}_validation.json` — handler diagnostics and metrics
- `{stem}_full_data.json` — complete info dict with derived metrics (Shannon entropy, invariants)
- `{stem}_model_trace.npz` — compressed NumPy archive of all extracted tensor parameters
- `{stem}_execution_report.md` — auto-generated Markdown report with config, mathematical invariant audits, Performance Insights scalar table, and embedded visualization PNGs
...
📖 **Related Documentation:** [MkDocs API Reference](../../docs-mkdocs/index.md) | [Guide: Model Structure](../../docs-mkdocs/guides/generative-model-structure.md)
...
| Auto-outputs   | `_full_data.json`, `_model_trace.npz`, `_execution_report.md` |
...
📖 **Related Documentation:** [State Inference API](../../docs-mkdocs/api/inference.md)
...
| Derived        | Shannon entropy `H_qs`, invariant validation on `qs` and `qpi`          |
| Auto-outputs   | `_full_data.json`, `_model_trace.npz`, `_execution_report.md` (with Performance Insights: H(q), -G) |
...
📖 **Related Documentation:** [Guide: Pymdp Environments](../../docs-mkdocs/guides/pymdp-env.md) | [Guide: Rollout Loop](../../docs-mkdocs/guides/rollout-active-inference-loop.md)
...
| Derived        | Shannon entropy `H_qs` trajectory, invariant validation on all matrices                       |
| Auto-outputs   | `_full_data.json`, `_model_trace.npz` (all rollout tensors), `_execution_report.md` (with Performance Insights) |
...
📖 **Related Documentation:** [SI Planning API](../../docs-mkdocs/api/planning-si.md) | [MCTS Planning API](../../docs-mkdocs/api/planning-mcts.md)
...
📖 **Related Documentation:** [Control / Inductive API](../../docs-mkdocs/api/control.md)
...
**Diagnostics:** `I_matrix` (full values), `H_vector`, `B_shape`, `threshold`, `depth`. **Auto-outputs:** `_model_trace.npz` (with I_matrix natively archived), `_execution_report.md`. If `I_matrix` is 2D+, triggers `plot_reachability_matrix` heatmap. **Upstream:** `test_inductive_inference_jax.py`.

---

## Category: Inference & Learning (2 examples)

📖 **Related Documentation:** [State Inference Reference](../../docs-mkdocs/api/inference.md) | [Dirichlet Learning Reference](../../docs-mkdocs/api/learning.md)
...
**Upstream for all legacy:** `test_demos.py`, `test_grid_world_parity.py`, `test_vfe_jax.py`. Legacy handlers that log `VFE` or `F` arrays automatically trigger `plot_free_energy` VFE line plots and report terminal VFE values in the Performance Insights table.
...
| VFE trajectory         | `_vfe.png`               | varies | `plot_free_energy`                 |
| Reachability matrix    | `_reachability_I.png`    | varies | `plot_reachability_matrix`         |
| A/B/C/D matrices       | `_matrix_{A,B,C,D}.png`  | varies | `plot_likelihood_matrix` etc.      |
| Action bars            | `_actions.png`           | varies | `plot_action_probabilities`        |
| Action donut           | `_action_donut.png`      | varies | `plot_action_frequency_donut`      |
| EFE breakdown          | `_G_components.png`      | varies | `plot_efe_components`              |
| SPM log comparison     | `_spm_log.png`           | varies | `plot_spm_log_comparison`          |
| Sparse structure       | `_sparse.png`            | varies | `plot_sparse_structure`            |

### Auto-Generated Data Artifacts

| Artifact Type        | File Suffix              | Content                                                      |
| -------------------- | ------------------------ | ------------------------------------------------------------ |
| Validation JSON      | `_validation.json`       | Handler return dict (`ok`, `id`, diagnostics)                |
| Full data JSON       | `_full_data.json`        | Complete info dict with derived `H_qs`, `_invariants`        |
| Native trace archive | `_model_trace.npz`       | Unrestricted compressed NumPy archive of all tensor keys     |
| Execution report     | `_execution_report.md`   | Markdown with config, invariants, Performance Insights, PNGs |
...
| Sphinx Path                                                           | Topic                        |
| --------------------------------------------------------------------- | ---------------------------- |
| `docs-mkdocs/tutorials/notebooks/pymdp_fundamentals.ipynb`            | Package fundamentals         |
| `docs-mkdocs/tutorials/notebooks/active_inference_from_scratch.ipynb` | Active inference walkthrough |
| `docs-mkdocs/tutorials/notebooks/using_the_agent_class.ipynb`         | `Agent` API usage            |
